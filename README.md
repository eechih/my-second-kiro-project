...
以 **訂單為核心** 的電子商務後台系統，提供訂單、進貨採購、出貨與庫存的整合管理。  
前端採用 React SPA；後端則基於 **AWS Amplify Gen2**（AppSync + DynamoDB + Lambda + S3 + Cognito）。
...
- **商品管理**
  - 商品 CRUD
  - 多張照片上傳（S3，含前端壓縮與自動縮圖）
  - 多維度規格組合（如顏色 × 尺寸）
  - 每個規格組合具備獨立 SKU、庫存，並可選擇覆寫單價/成本
...
商品可定義多組規格維度（如顏色、尺寸），系統自動產生笛卡爾積組合（如「黑 L」、「白 XL」）。  
每個組合具備獨立 SKU、庫存與可選的單價/成本覆寫；庫存追蹤在規格組合層級操作。
...
- **圖片優化**：上傳前 Canvas 壓縮、Lambda + `sharp` 自動產生縮圖、S3 物件標籤（tags）
