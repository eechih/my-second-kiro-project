...
const variantArb: fc.Arbitrary<ProductVariant> = fc
  .record({
    price: fc.option(positiveNumberArb, { nil: null }),
    cost: fc.option(positiveNumberArb, { nil: null }),
  })
  .map(({ price, cost }) => ({
    id: "var-1",
    label: "黑 L",
...
describe("屬性 18：規格組合價格/成本解析——覆寫優先", () => {
  it("price 不為 null 時，resolveEffectivePrice 應回傳覆寫值", () => {
    fc.assert(
  ...
  it("price 為 null 時，resolveEffectivePrice 應回傳商品預設單價", () => {
    fc.assert(
  ...
  it("cost 不為 null 時，resolveEffectiveCost 應回傳覆寫值", () => {
    fc.assert(
  ...
  it("cost 為 null 時，resolveEffectiveCost 應回傳商品預設成本", () => {
    fc.assert(
      fc.property(productArb, (product) => {
        const variant: ProductVariant = {
          id: "var-1",
    ...
    fc.assert(
      fc.property(variantArb, productArb, (variant, product) => {
        ...
        if (variant.price !== null) {
          expect(price).toBe(variant.price);
        } else {
          expect(price).toBe(product.unitPrice);
        }

        if (variant.cost !== null) {
          expect(cost).toBe(variant.cost);
        } else {
