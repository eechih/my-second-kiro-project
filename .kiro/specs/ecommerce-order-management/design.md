...
- **列表頁面 UI 模式**：客戶與供應商列表頁面使用獨立的 TanStack Table + MUI Table 實作（不使用共用 `DataTable` 元件），搭配專屬的 `-components/` 子目錄放置 Toolbar、InfoCell、CursorPagination、RowActions 等子元件。此模式提供更精細的 UI 控制（批次選取、複合欄位、CSV 匯出、排序下拉選單），且避免修改共用 `DataTable` 影響其他頁面。商品與訂單列表頁面可視需求沿用此模式或使用共用 `DataTable`。
- **純函式工具模組**：將 Avatar 色彩衍生（`src/lib/avatar-utils.ts`）、列號計算與排序（`src/lib/table-utils.ts`）、CSV 匯出（`src/lib/customer-csv.ts`、`src/lib/supplier-csv.ts`）等邏輯抽離為純函式，方便獨立測試與跨頁面複用。
- **TanStack Query 快取策略**：
...
│   ├── index.tsx              # 儀表板首頁
│   ├── customers/             # 客戶管理頁面
│   │   ├── index.tsx          # 客戶列表（獨立 TanStack Table + MUI Table）
│   │   ├── new.tsx            # 新增客戶
│   │   ├── $customerId.tsx    # 編輯客戶
│   │   └── -components/       # 客戶列表子元件（非路由目錄）
│   │       ├── CustomerToolbar.tsx
│   │       ├── UserInfoCell.tsx
│   │       ├── CursorPagination.tsx
│   │       └── RowActions.tsx
│   ├── suppliers/             # 供應商管理頁面
│   │   ├── index.tsx          # 供應商列表（獨立 TanStack Table + MUI Table）
│   │   ├── new.tsx            # 新增供應商
│   │   ├── $supplierId.tsx    # 編輯供應商
│   │   └── -components/       # 供應商列表子元件（非路由目錄）
│   │       ├── SupplierToolbar.tsx
│   │       ├── SupplierInfoCell.tsx
│   │       ├── CursorPagination.tsx
│   │       └── SupplierRowActions.tsx
│   ├── products/              # 商品管理頁面
│   │   ├── index.tsx          # 商品列表
│   │   ├── new.tsx            # 新增商品
│   │   └── $productId.tsx     # 編輯商品
...
│   ├── useCustomerListCursor.ts # 客戶列表游標式分頁查詢 hook
│   ├── useSuppliers.ts        # 供應商 CRUD hooks
│   ├── useSupplierListCursor.ts # 供應商列表游標式分頁查詢 hook
...
├── components/                # 共用 UI 元件
│   ├── DataTable.tsx           # 通用分頁表格元件（TanStack Table + MUI，供商品/訂單等頁面使用）
│   ├── SearchBar.tsx           # 搜尋列元件
...
├── lib/                       # 工具函式
│   ├── amplify-client.ts      # Amplify client 封裝
│   ├── avatar-utils.ts        # Avatar 色彩衍生與字元取得
│   ├── table-utils.ts         # 列號計算、排序工具
│   ├── customer-csv.ts        # 客戶 CSV 匯出純函式
│   └── supplier-csv.ts        # 供應商 CSV 匯出純函式
├── auth/
...
- `CustomerListPage`：獨立的 TanStack Table + MUI Table 實作（不使用共用 `DataTable`），整合 `CustomerToolbar`（搜尋、狀態篩選、排序、新增按鈕、CSV 匯出）、`UserInfoCell`（Avatar + 名稱/聯絡人複合欄位）、`CursorPagination`（游標式分頁）、`RowActions`（檢視/編輯/停用啟用按鈕）。支援批次選取、麵包屑導覽。使用 `useCursorPagination` hook 管理分頁狀態，使用 `useCustomerListCursor` hook 取得資料。
- `CustomerFormPage`：新增/編輯表單，使用 TanStack Form

**子元件（`src/routes/customers/-components/`）：**

- `CustomerToolbar`：單列式工具列，水平排列搜尋輸入框（含防抖）、Status_Filter（MUI Select：全部狀態/啟用中/已停用）、Sort_Dropdown（MUI Select：客戶名稱/聯絡人/電話/建立日期）、新增客戶按鈕、CSV 匯出 IconButton
- `UserInfoCell`：Avatar（`getAvatarLetter` + `getAvatarColor`）+ 客戶名稱（subtitle2 粗體）+ 聯絡人（body2 次要色彩）
- `CursorPagination`：每頁筆數下拉選單（10/25/50）、顯示筆數文字、上一頁/下一頁按鈕
- `RowActions`：檢視（VisibilityIcon）、編輯（EditIcon）、停用/啟用（BlockIcon warning / CheckCircleIcon success）
...
// useCustomerListCursor.ts — 供列表頁面使用的游標式分頁查詢 hook
function useCustomerListCursor(params: {
  pageSize: number;
  nextToken?: string;
  search?: string;
  isActive?: boolean; // undefined = 全部
  sortField?: "name" | "contactPerson" | "phone" | "createdAt";
}): UseQueryResult<PaginatedResult<Customer>>;
```
...
- `SupplierListPage`：獨立的 TanStack Table + MUI Table 實作（不使用共用 `DataTable`），整合 `SupplierToolbar`（搜尋、狀態篩選、排序、新增按鈕、CSV 匯出）、`SupplierInfoCell`（Avatar + 名稱/聯絡人複合欄位）、`CursorPagination`（游標式分頁）、`SupplierRowActions`（檢視/編輯/停用啟用按鈕）。支援批次選取、麵包屑導覽。使用 `useCursorPagination` hook 管理分頁狀態，使用 `useSupplierListCursor` hook 取得資料。
- `SupplierFormPage`：新增/編輯表單

**子元件（`src/routes/suppliers/-components/`）：**

- `SupplierToolbar`：單列式工具列，水平排列搜尋輸入框（含防抖）、Status_Filter（MUI Select：全部狀態/啟用中/已停用）、Sort_Dropdown（MUI Select：供應商名稱/聯絡人/電話/建立日期）、新增供應商按鈕、CSV 匯出 IconButton
- `SupplierInfoCell`：Avatar（`getAvatarLetter` + `getAvatarColor`）+ 供應商名稱（subtitle2 粗體）+ 聯絡人（body2 次要色彩）
- `CursorPagination`：每頁筆數下拉選單（10/25/50）、顯示筆數文字、上一頁/下一頁按鈕
- `SupplierRowActions`：檢視（VisibilityIcon）、編輯（EditIcon）、停用/啟用（BlockIcon warning / CheckCircleIcon success）
...
// useSupplierListCursor.ts — 供列表頁面使用的游標式分頁查詢 hook
function useSupplierListCursor(params: {
  pageSize: number;
  nextToken?: string;
  search?: string;
  isActive?: boolean; // undefined = 全部
...
// 注意：客戶與供應商列表頁面已改用獨立的 TanStack Table + MUI Table 實作（含專屬 Toolbar、InfoCell 等子元件），
// 不再使用此共用元件。此元件仍供商品列表、訂單列表等其他頁面使用。
...
| 屬性    | 測試目標           | 測試檔案                                                     |
| ------- | ------------------ | ------------------------------------------------------------ |
| 屬性 1  | 實體驗證函式       | `shared/logic/__tests__/validation.property.test.ts`         |
| 屬性 2  | 訂單狀態轉換       | `shared/logic/__tests__/order-status.property.test.ts`       |
| 屬性 3  | 明細狀態轉換       | `shared/logic/__tests__/line-item-status.property.test.ts`   |
| 屬性 4  | 採購記錄狀態轉換   | `shared/logic/__tests__/purchase-record.property.test.ts`    |
| 屬性 5  | 金額計算           | `shared/logic/__tests__/order-calculations.property.test.ts` |
| 屬性 6  | 採購數量守恆       | `shared/logic/__tests__/purchase-record.property.test.ts`    |
| 屬性 7  | 出貨驗證           | `shared/logic/__tests__/shipment.property.test.ts`           |
| 屬性 8  | 入庫庫存增加       | `shared/logic/__tests__/shipment.property.test.ts`           |
| 屬性 9  | 出貨庫存減少       | `shared/logic/__tests__/shipment.property.test.ts`           |
| 屬性 10 | 訂單狀態自動推導   | `shared/logic/__tests__/order-status.property.test.ts`       |
| 屬性 11 | 狀態歷史記錄       | `shared/logic/__tests__/order-status.property.test.ts`       |
| 屬性 12 | 訂單合併守恆       | `shared/logic/__tests__/order-merge.property.test.ts`        |
| 屬性 13 | 合併前置驗證       | `shared/logic/__tests__/order-merge.property.test.ts`        |
| 屬性 14 | 訂單分拆守恆       | `shared/logic/__tests__/order-split.property.test.ts`        |
| 屬性 15 | 分拆前置驗證       | `shared/logic/__tests__/order-split.property.test.ts`        |
| 屬性 16 | 序列化往返         | `shared/logic/__tests__/serialization.test.ts`               |
| 屬性 17 | 規格組合產生正確性 | `shared/logic/__tests__/product-variant.property.test.ts`    |
| 屬性 18 | 價格/成本解析      | `shared/logic/__tests__/product-variant.property.test.ts`    |
| 屬性 19 | 規格組合必選驗證   | `shared/logic/__tests__/product-variant.property.test.ts`    |
...
src/
├── lib/__tests__/
│   ├── avatar-utils.property.test.ts
│   ├── table-utils.property.test.ts
│   ├── customer-csv.property.test.ts
│   └── supplier-csv.property.test.ts
├── hooks/__tests__/
│   ├── useCursorPagination.property.test.ts
│   └── useSelection.property.test.ts
...
├── routes/customers/__tests__/
│   ├── CustomerToolbar.test.tsx
│   ├── UserInfoCell.test.tsx
│   ├── CursorPagination.test.tsx
│   ├── RowActions.test.tsx
│   └── Breadcrumb.test.tsx
├── routes/suppliers/__tests__/
│   ├── SupplierToolbar.test.tsx
│   ├── SupplierInfoCell.test.tsx
│   ├── CursorPagination.test.tsx
│   ├── SupplierRowActions.test.tsx
│   └── Breadcrumb.test.tsx
└── routes/__tests__/
    ├── products.test.tsx
    └── orders.test.tsx
