...
- [ ] 1. 建立純函式工具模組
  ...
  - [x] 1.3 建立 `src/lib/customer-csv.ts`
...
- [ ] 2. 建立游標分頁 Hook
  - [x] 2.1 建立 `src/hooks/useCursorPagination.ts`
...
- [ ] 3. 建立客戶列表資料查詢 Hook
  - [x] 3.1 建立 `src/hooks/useCustomerListCursor.ts`
...
- [x] 4. Checkpoint - 確認純函式與 Hooks 正確
  - 確保所有測試通過，如有問題請詢問使用者。

- [ ] 5. 建立 UI 子元件
  - [x] 5.1 建立 `src/routes/customers/components/CustomerToolbar.tsx`
  ...
  - [x] 5.2 建立 `src/routes/customers/components/UserInfoCell.tsx`
  ...
  - [x] 5.3 建立 `src/routes/customers/components/CursorPagination.tsx`
  ...
  - [x] 5.4 建立 `src/routes/customers/components/RowActions.tsx`
...
- [x] 6. Checkpoint - 確認 UI 子元件正確
  - 確保所有測試通過，如有問題請詢問使用者。

- [ ] 7. 重構客戶列表頁面
  - [x] 7.1 重構 `src/routes/customers/index.tsx` — 整合所有新元件
